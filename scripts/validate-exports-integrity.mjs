	...
	for (const [pkgName, config] of publishedExports.entries()) {
		for (const filteredPath of config.filteredExports) {
			...
			const unconditionalPatterns = [
				...
				new RegExp(
					`import\\s+[^;]*from\\s+['"]${pkgName.replace(/\//g, "\\/")}${filteredPath.replace(/\//g, "\\/")[0] === "/" ? filteredPath.replace(/\//g, "\\/") : ""}['"];`,
					"g",
				),
				// require('@pkg/filtered')
				new RegExp(
					`require\\s*\\(\\s*['"]${pkgName.replace(/\//g, "\\/")}${filteredPath.replace(/\//g, "\\/")[0] === "/" ? filteredPath.replace(/\//g, "\\/") : ""}['"]\\s*\\)`,
					"g",
				),
			];
