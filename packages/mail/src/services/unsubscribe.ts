...
if (!TOKEN_SECRET) {
	logger.warn("⚠️  UNSUBSCRIBE_TOKEN_SECRET or BETTER_AUTH_SECRET not configured - token generation will fail");
}
 ...
 */
export async function getEmailPreferences(db: Database, userId: string): Promise<EmailPreference | null> {
	try {
 ...
 * Based on email_system_critique.md
 */
export async function createDefaultPreferences(db: Database, userId: string, email: string): Promise<EmailPreference> {
	try {
 ...
 * Based on email_system_critique.md
 */
export async function unsubscribe(db: Database, userId: string, category?: EmailCategory): Promise<void> {
	try {
 ...
 */
export async function resubscribe(db: Database, userId: string, category?: "achievement" | "nurture"): Promise<void> {
	try {
 ...
 */
export async function canReceiveEmail(db: Database, userId: string, category: EmailCategory): Promise<boolean> {
	try {
		// Transactional and operational emails always sent
...
export async function syncUnsubscribeToHubSpot(email: string, unsubscribed: boolean): Promise<void> {
	try {
		if (!process.env.HUBSPOT_ACCESS_TOKEN) {
			logger.debug("HUBSPOT_ACCESS_TOKEN not configured - skipping HubSpot sync");
			return;
		}
 ...
 */
export async function syncPreferenceToHubSpot(email: string, category: EmailCategory, enabled: boolean): Promise<void> {
	try {
		if (!process.env.HUBSPOT_ACCESS_TOKEN) {
			logger.debug("HUBSPOT_ACCESS_TOKEN not configured - skipping HubSpot sync");
			return;
