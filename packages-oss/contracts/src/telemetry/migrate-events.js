...
import { TelemetryEventMapper, mapLegacyEventsToCore } from "./event-mapper";
...
function main() {
    ...
    try {
        ...
        const legacyEvents = JSON.parse(inputData);
 ...
 */
async function migrateLegacyEventsToFile(inputFile, outputFile) {
    const inputData = readFileSync(inputFile, "utf8");
    const legacyEvents = JSON.parse(inputData);
 ...
 */
function migrateLegacyEventsToArray(legacyEvents) {
