...
export function TeamsSection() {
	...
	return (
		<section className="py-32 bg-background relative overflow-hidden" aria-labelledby="teams-heading">
			<div className="absolute inset-0 bg-gradient-to-b from-transparent via-card to-transparent opacity-50" aria-hidden="true" />
				...
				<div className="grid md:grid-cols-3 gap-8" role="list">
					{for_teams.content.metric_cards.map((card, index) => (
						<m.div
							...
							role="listitem"
						>
							<div className="absolute inset-0 bg-gradient-to-b from-border to-card rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 motion-reduce:transition-none" aria-hidden="true" />
							<div className="relative p-8 rounded-2xl border border-border bg-background/50 backdrop-blur-sm group-hover:border-border/60 transition-colors motion-reduce:transition-none h-full">
								<div className="text-4xl font-bold text-foreground mb-4" aria-label={`Metric: ${card.title}`}>
									{index === 0 && (
										<>
