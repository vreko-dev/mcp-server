...
describe('Pre-Publish Automation', () => {
  ...
  describe('Git State Capture', () => {
    it('should capture current git state', () => {
      vi.mocked(execSync).mockImplementation((cmd: string) => {
        ...
        if (cmd.includes('rev-parse HEAD')) return Buffer.from('abc123def456\n');
        if (cmd.includes('diff --cached --name-only')) return Buffer.from('file1.ts\nfile2.ts\n');
        if (cmd.includes('diff --name-only')) return Buffer.from('file3.ts\n');
        return Buffer.from('');
      });
  ...
  describe('Rollback Scenarios', () => {
    ...
    it('should preserve git state during rollback', () => {
      vi.mocked(execSync).mockImplementation((cmd: string) => {
        if (cmd.includes('git reset --hard')) return Buffer.from('');
        if (cmd.includes('git clean -fd')) return Buffer.from('');
        return Buffer.from('');
      });
