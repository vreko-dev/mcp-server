...
function ContentMenuItem({ label, path, isPage, children, activePath }: ContentStructureItem & { activePath: string }) {
	return (
		<>
			{isPage ? (
				<Link
			...
			{children && (
				<div className="-mr-4 pl-4">
					{children.map((subItem) => (
						<ContentMenuItem key={subItem.path} {...subItem} activePath={activePath} />
					))}
				</div>
			)}
		</>
...
export function ContentMenu({ items, activePath }: { items: ContentStructureItem[]; activePath: string }) {
	return (
