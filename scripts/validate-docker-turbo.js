...
if (missingDockerfiles.length === 0) {
  ...
  console.log(`   ✗ FAILED: Missing Dockerfiles: ${missingDockerfiles.join(', ')}\n`);
  allPassed = false;
}
...
const dockerfilesToCheck = [
  { path: 'apps/api/Dockerfile.prod', package: '@snapback/api-service' },
  { path: 'apps/mcp-server/Dockerfile.prod', package: '@snapback/mcp-server' },
...
for (const { path: dockerPath, package: pkgName } of dockerfilesToCheck) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (content.includes(`turbo prune ${pkgName}`)) {
      filesWithTurboPrune++;
      console.log(`   ✓ ${dockerPath}: turbo prune configured for ${pkgName}`);
    } else {
      console.log(`   ✗ ${dockerPath}: Missing turbo prune for ${pkgName}`);
      allPassed = false;
    }
  }
}

if (filesWithTurboPrune === dockerfilesToCheck.length && filesWithPrunerStage === dockerfilesToCheck.length) {
...
for (const { path: dockerPath } of dockerfilesToCheck) {
  ...
  if (fs.existsSync(filePath)) {
    const content = fs.readFileSync(filePath, 'utf-8');
    const missingStages = requiredStages.filter(stage => !content.includes(`AS ${stage}`));
    
    if (missingStages.length === 0) {
      console.log(`   ✓ ${dockerPath}: All stages present (${requiredStages.join(', ')})`);
    } else {
      console.log(`   ✗ ${dockerPath}: Missing stages: ${missingStages.join(', ')}`);
...
for (const { path: dockerPath } of dockerfilesToCheck) {
  ...
  if (fs.existsSync(filePath)) {
    const content = fs.readFileSync(filePath, 'utf-8');
    const missingFeatures = securityFeatures.filter(({ pattern }) => !pattern.test(content));
    
    if (missingFeatures.length === 0) {
      console.log(`   ✓ ${dockerPath}: All security features present`);
    } else {
      console.log(`   ✗ ${dockerPath}: Missing: ${missingFeatures.map(f => f.name).join(', ')}`);
...
for (const { path: pkgPath, name } of packagesToCheck) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    const hasDockerBuildDev = pkg.scripts && pkg.scripts['docker-build:dev'];
    const usesProdDockerfile = hasDockerBuild && pkg.scripts['docker-build'].includes('Dockerfile.prod');
    
    if (hasDockerBuild && hasDockerBuildDev && usesProdDockerfile) {
      console.log(`   ✓ ${name}: docker-build and docker-build:dev scripts present`);
    } else {
      console.log(`   ✗ ${name}: Missing docker build scripts or not using Dockerfile.prod`);
...
if (fs.existsSync(nextConfigPath)) {
  ...
  if (nextConfig.includes('output: "standalone"')) {
    console.log('   ✓ Next.js standalone output enabled for Docker optimization\n');
  } else {
...
for (const { path: dockerPath } of dockerfilesToCheck) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (content.includes('--mount=type=cache') && content.includes('/pnpm/store')) {
...
if (allPassed) {
  ...
  console.log('\n🚀 Ready to build:');
  console.log('  pnpm --filter @snapback/web docker-build       # Build web (prod)');
  console.log('  pnpm --filter @snapback/api docker-build       # Build API (prod)');
  console.log('  pnpm --filter @snapback/mcp-server docker-build  # Build MCP (prod)');
  console.log('  pnpm docker:affected                           # Build all changed (via Turbo)');
