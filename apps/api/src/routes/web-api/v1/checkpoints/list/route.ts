...
async function handleDeviceTrialList(
	...
	try {
		...
		const snapshotsResponse = snapshotList.map((snapshot) => ({
		...
		return NextResponse.json({
			checkpoints: snapshotsResponse,
			totalCount,
		});
	} catch (error) {
		logger.error("Device trial checkpoint listing error", { error });
		return NextResponse.json(
...
async function handleAuthenticatedUserList(
	...
	try {
		...
		logger.error("Authenticated user checkpoint listing error", { error });
		return NextResponse.json(
			{ error: "Failed to list checkpoints" },
			{ status: 500 },
		);
