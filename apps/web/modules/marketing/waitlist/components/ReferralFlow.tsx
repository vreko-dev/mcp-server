...
import { Copy, Check, Twitter, MessageSquare, ArrowRight } from "lucide-react";
        ...
        <div className="py-4 space-y-2">
          ...
          <div className="flex justify-between pt-2">
             {REFERRAL_MILESTONES.map((m) => (
               <div key={m.count} className={`text-xs flex flex-col items-center gap-1 ${currentReferrals >= m.count ? 'text-green-400' : 'text-gray-500'}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-black/20 rounded-lg p-4 border border-white/5">
           <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-mono text-snapback-text-secondary uppercase">Discord / Slack Copy</span>
              <button onClick={() => { navigator.clipboard.writeText(shareText); setCopied(true); }} className="text-xs text-snapback-green hover:underline">
                 Copy Text
              </button>
           </div>
