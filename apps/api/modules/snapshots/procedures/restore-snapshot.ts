...
export const restoreSnapshot = protectedProcedure.input(restoreSnapshotSchema).handler(async ({ input, context }) => {
	...
	const highRiskFiles = filesToRestore.filter((f) => f.riskLevel === "high" || f.containsSecrets);
	const mediumRiskFiles = filesToRestore.filter((f) => f.riskLevel === "medium");

	const riskAssessment = {
		totalFiles: filesToRestore.length,
		highRiskCount: highRiskFiles.length,
		mediumRiskCount: mediumRiskFiles.length,
		lowRiskCount: filesToRestore.length - highRiskFiles.length - mediumRiskFiles.length,
		containsSecrets: highRiskFiles.some((f) => f.containsSecrets),
		overallRisk: highRiskFiles.length > 0 ? "high" : mediumRiskFiles.length > 0 ? "medium" : "low",
	};
