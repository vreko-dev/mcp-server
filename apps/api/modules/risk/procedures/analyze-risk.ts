...
export const analyzeRisk = protectedProcedure.input(analyzeRiskSchema).handler(async ({ input, context }) => {
	...
	for (const file of input.files) {
		...
		if (file.linesDeleted && file.totalLines && file.linesDeleted > 100) {
			...
			const severity: "low" | "medium" | "high" = percentage > 80 ? "high" : percentage > 50 ? "medium" : "low";
	...
	if (permissions.advancedDetection) {
		...
		const totalChanges = input.files.reduce((sum, f) => sum + (f.linesAdded || 0) + (f.linesDeleted || 0), 0);
	...
	} else if (input.files.some((f) => (f.linesAdded || 0) + (f.linesDeleted || 0) > 300)) {
	...
	const riskLevel: "low" | "medium" | "high" = finalRiskScore < 30 ? "low" : finalRiskScore < 70 ? "medium" : "high";
