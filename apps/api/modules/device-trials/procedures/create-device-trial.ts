...
export const createDeviceTrial = publicProcedure
	.input(createDeviceTrialSchema)
	.handler(async ({ input }) => {
		...
		if (existingTrial && existingTrial.length > 0) {
			...
			if (trial.blockedUntil && trial.blockedUntil.getTime() <= Date.now()) {
				...
				const apiKeyResult = await db
					.select()
					.from(apiKeys)
					.where(eq(apiKeys.id, trial.apiKeyId))
					.limit(1);

				const apiKey =
					apiKeyResult && apiKeyResult.length > 0 ? apiKeyResult[0] : undefined;
			...
			if (newInstallCount > 3) {
				...
				const hoursSinceCreation =
					(Date.now() - trial.createdAt.getTime()) / (1000 * 60 * 60);
		...
		const trialResult = await db
			...
			.returning();
		const trial =
			trialResult && trialResult.length > 0 ? trialResult[0] : undefined;
