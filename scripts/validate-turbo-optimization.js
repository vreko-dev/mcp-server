...
const hasGlobalEnv = Array.isArray(turboConfig.globalEnv) && turboConfig.globalEnv.includes("DATABASE_URL");
const hasPassThrough =
	Array.isArray(turboConfig.globalPassThroughEnv) && turboConfig.globalPassThroughEnv.includes("DOCKER_BUILDKIT");
...
const requiredScripts = ["deploy:affected", "docker:affected", "test:affected", "build:affected"];
...
const hasDeployScripts =
	webPkg.scripts.deploy && apiPkg.scripts.deploy && mcpPkg.scripts.deploy && docsPkg.scripts.deploy;

const hasDockerBuildScripts =
	webPkg.scripts["docker-build"] && apiPkg.scripts["docker-build"] && mcpPkg.scripts["docker-build"];
