import { mergeConfigs, nodeConfig, TEST_TIMEOUTS } from "@snapback/vitest-config";
import { defineProject } from "vitest/config";

/**
 * Vitest configuration for @snapback/engine
 * Uses shared nodeConfig preset from @snapback/vitest-config
 * Has custom coverage thresholds per module
 */
export default defineProject(
	mergeConfigs(nodeConfig, {
		test: {
			...
			testTimeout: TEST_TIMEOUTS.integration, // 30 seconds for tests with coverage overhead
			coverage: {
				provider: "v8",
				include: ["src/**/*.ts"],
				exclude: [
