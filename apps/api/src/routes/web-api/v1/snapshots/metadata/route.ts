...
/**
 * POST /api/v1/snapshots/metadata
 *
 * Creates and stores snapshot metadata
 */
...
async function handleDeviceTrialCheckpoint(authContext: any, body: any) {
	try {
		...
		logger.error("Device trial snapshot creation error", { error });
		return NextResponse.json(
			{ error: "Failed to create snapshot" },
			{ status: 500 },
		);
...
async function handleAuthenticatedUserCheckpoint(
	...
	try {
		...
		logger.error("Authenticated user snapshot creation error", { error });
		return NextResponse.json(
			{ error: "Failed to create snapshot" },
			{ status: 500 },
		);
