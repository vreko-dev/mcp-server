...
const DEFAULT_POLICY = {
	...
	pathRules: [] as { glob: string; thresholds: any }[],
};
 ...
 * @param issueCounts Count of issues by severity
...
function calculateConfidence(
	...
	// Adjust confidence based on how close we are to thresholds
	...
	// Reduce confidence as we get closer to thresholds
 ...
 * @param result SARIF result
 * @returns Severity level
 */
function getSeverity(result: any): string {
	// Map SARIF levels to our severity categories
	// SARIF uses: "error", "warning", "note"
	// We map to: "critical", "high", "medium", "low"
	// Also support direct severity values from tests

	if (result.level) {
		const level = result.level.toLowerCase();
		switch (level) {
			// SARIF standard levels
			case "error":
				return "critical";
			case "warning":
				return "high";
			case "note":
				return "medium";
			// Direct severity levels (used in tests and some tools)
			case "critical":
				return "critical";
			case "high":
				return "high";
	...
	if (result.ruleId && result.ruleIndex !== undefined) {
		// In a real implementation, we would look up the rule in the SARIF run
...
function matchesGlob(filePath: string, glob: string): boolean {
	// This is a simplified glob matching implementation
	// In a real implementation, we would use a proper glob matching library

	// Handle the src/** pattern specifically
	if (glob === "src/**") {
