...
const EnrichEventSchema = z.discriminatedUnion("type", [
	DecisionEventSchema,
	OutcomeEventSchema,
]);

export const enrichEvent = protectedProcedure
	.input(EnrichEventSchema)
	.handler(async ({ input, context }) => {
				...
				try {
					const existing = await db
						...
						.where(
							and(
								eq(
									snapbackSchema.telemetryIdempotencyKeys.idempotencyKey,
									input.idempotencyKey,
								),
								gt(
									snapbackSchema.telemetryIdempotencyKeys.expiresAt,
									new Date(),
								),
							),
						)
		...
		const enrichedEvent = {
			...
			eventCategory:
				input.type === "decision" ? "risk_analysis" : "user_action",
			properties: input,
			platform: "vscode",
