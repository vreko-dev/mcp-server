...
describe("Phase 1: SDK Migration", () => {
  ...
  describe("Import Updates", () => {
    it("should NOT import from @snapback/infrastructure", () => {
      ...
      try {
        const result = execSync(
          `grep -r "from.*@snapback/infrastructure" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should NOT import from @snapback/contracts (old)", () => {
      ...
      try {
        const result = execSync(
          `grep -r "from.*@snapback/contracts" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should import from @snapback-oss/infrastructure", () => {
      ...
      try {
        const result = execSync(
          `grep -r "@snapback-oss/infrastructure" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should import from @snapback-oss/contracts", () => {
      ...
      try {
        const result = execSync(
          `grep -r "@snapback-oss/contracts" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should NOT have better-sqlite3 imports or requires", () => {
      ...
      try {
        const result = execSync(
          `grep -r "better-sqlite3" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
  ...
  describe("No Private Dependencies", () => {
    it("should NOT import from @snapback/platform", () => {
      ...
      try {
        const result = execSync(
          `grep -r "@snapback/platform" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should NOT import from @snapback/analytics", () => {
      ...
      try {
        const result = execSync(
          `grep -r "@snapback/analytics[^-]" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
    ...
    it("should NOT import from @snapback/core", () => {
      ...
      try {
        const result = execSync(
          `grep -r "@snapback/core" ${join(OSS_SDK, "src")} || true`,
          { encoding: "utf-8" }
        );
