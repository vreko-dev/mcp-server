...
export default function UserStart() {
	...
	const demoFileIds = useMemo(() => ["file-1", "file-2", "file-3", "file-4", "file-5"], []);
	...
	const onProtectionStatusChange = useCallback((fileId: string, newStatus: "enabled" | "disabled") => {
		const message = newStatus === "enabled" ? `${fileId} protection enabled` : `${fileId} protection disabled`;
	...
	const computedMetrics = useMemo(() => {
		const protectedCount = Array.from(protectionStatuses.values()).filter((s) => s.protection === "enabled").length;
