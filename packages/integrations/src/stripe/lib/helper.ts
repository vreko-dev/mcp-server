...
function getActivePlanFromPurchases(purchases?: PurchaseWithoutTimestamps[]) {
	...
	if (subscriptionPurchase) {
		const planEntry = Object.entries(plans).find(([_, plan]) =>
			plan.prices?.some((price: { productId: string }) => price.productId === subscriptionPurchase.productId),
		);

		return {
			id: planEntry?.[0] as PlanId | undefined,
			price: planEntry?.[1].prices?.find(
	...
	if (oneTimePurchase) {
		const planEntry = Object.entries(plans).find(([_, plan]) =>
			plan.prices?.some((price: { productId: string }) => price.productId === oneTimePurchase.productId),
		);
