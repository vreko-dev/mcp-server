 ...
 * NOTE: This service is internal to @snapback/core and uses lazy loading
 * to avoid circular dependency issues with @snapback/platform.
 * It is not exported in the public SDK and is used only internally.
 *
 * @module DeviceTrialService
 * @package @snapback/core (private)
 */
...
export class DeviceTrialService {
	...
	constructor(private db: any) {
		...
		this.logger = require("@snapback/infrastructure").logger;
		this.nanoid = require("nanoid").nanoid;
		const platform = require("@snapback/platform");
		this.deviceTrials = platform.deviceTrials;
		this.apiKeys = platform.apiKeys;
	}
	...
	private validateFingerprint(fingerprint: string): Result<void> {
		...
		if (!fingerprint || fingerprint.length === 0) {
			...
			this.logger.warn("Device trial validation failed", {
				code: error.code,
	...
	private generateApiKey(): { key: string; preview: string } {
		const key = `${DEVICE_TRIAL_CONSTANTS.API_KEY_PREFIX}_${this.nanoid(DEVICE_TRIAL_CONSTANTS.API_KEY_LENGTH)}`;
	...
	async createDeviceTrial(fingerprint: string): Promise<Result<DeviceTrialData>> {
		...
		try {
			...
			const existingTrials = await this.db
				.select()
				.from(this.deviceTrials)
				.where(eq(this.this.deviceTrials.deviceFingerprint, fingerprint))
				.limit(1);
			...
			if (existingTrials && existingTrials.length > 0) {
				...
				if (trial.blockedUntil && trial.blockedUntil.getTime() > Date.now()) {
					...
					this.logger.warn("Device trial creation blocked", {
						code: error.code,
						deviceId: trial.id,
				...
				if (trial.blockedUntil && trial.blockedUntil.getTime() <= Date.now()) {
					...
					await this.db
						.update(this.deviceTrials)
						.set({
							blockedUntil: null,
							installCount: updatedInstallCount,
							lastSeenAt: new Date(),
						})
						.where(eq(this.this.deviceTrials.id, trial.id));

					this.logger.info("Device trial block expired", {
						deviceId: trial.id,
						installCount: updatedInstallCount,
					});
				...
				if (newInstallCount > DEVICE_TRIAL_CONSTANTS.MAX_INSTALLS_IN_24H) {
					...
					if (hoursSinceCreation < 24) {
						...
						this.logger.error("Device trial abuse detected", {
							code: error.code,
							deviceId: trial.id,
							installCount: newInstallCount,
							blockedUntil: blockedUntil.toISOString(),
				...
				this.logger.debug("Device trial updated", { deviceId: trial.id, installCount: newInstallCount });
				return this.getTrialWithApiKey(trial.id);
			}
			...
			this.logger.error("Device trial creation database error", {
				code: dbError.code,
	...
	private async createNewTrial(fingerprint: string): Promise<Result<DeviceTrialData>> {
		try {
			...
			if (!newApiKey) {
				...
				this.logger.error("Device trial API key creation failed", { code: error.code });
				return { success: false, error };
			}
			...
			if (!trial) {
				...
				this.logger.error("Device trial record creation failed", { code: error.code });
				return { success: false, error };
			}

			this.logger.info("New device trial created", {
				deviceId: trial.id,
				apiKeyPreview,
	...
	private async getTrialWithApiKey(trialId: string): Promise<Result<DeviceTrialData>> {
		try {
			const trialResult = await this.db
				.select()
				.from(deviceTrials)
				.where(eq(this.deviceTrials.id, trialId))
				.limit(1);

			if (!trialResult || trialResult.length === 0) {
				const error = new DeviceTrialDatabaseError("Trial not found");
				this.logger.warn("Device trial lookup failed", { code: error.code, trialId });
				return { success: false, error };
			}
			...
			const apiKeyResult = await this.db
				.select()
				.from(apiKeys)
				.where(eq(this.apiKeys.id, trial.apiKeyId))
				.limit(1);
			...
			if (!apiKey) {
				...
				this.logger.error("Device trial API key lookup failed", { code: error.code, trialId });
				return { success: false, error };
			}
			...
			this.logger.error("Device trial retrieval database error", {
				code: dbError.code,
				cause: dbError.context?.cause,
			});
			return { success: false, error: dbError };
	...
	async validateQuota(fingerprint: string): Promise<Result<{ hasQuota: boolean }>> {
		try {
			const trials = await this.db
				.select()
				.from(deviceTrials)
				.where(eq(this.deviceTrials.deviceFingerprint, fingerprint))
				.limit(1);

			if (!trials || trials.length === 0) {
				this.logger.debug("Device trial not found for quota check", { fingerprint });
				return {
					success: true,
			...
			if (!hasQuota) {
				this.logger.warn("Device trial quota exhausted", {
					trialId: trial.id,
			...
			this.logger.error("Device trial quota validation database error", {
				code: dbError.code,
				cause: dbError.context?.cause,
			});
			return { success: false, error: dbError };
	...
	async linkDeviceToUser(fingerprint: string, userId: string): Promise<Result<{ linked: boolean }>> {
		try {
			...
			if (!trials || trials.length === 0) {
				const error = new DeviceTrialDatabaseError("Trial not found");
				this.logger.warn("Device trial not found for linking", { code: error.code, fingerprint, userId });
				return { success: false, error };
			}
			...
			await this.db
				...
				.where(eq(this.deviceTrials.id, trial.id));

			this.logger.info("Device trial converted to email tier", {
				trialId: trial.id,
				userId,
				snapshotLimit: DEVICE_TRIAL_CONSTANTS.CONVERTED_SNAPSHOT_LIMIT,
			});
			...
			this.logger.error("Device trial linking database error", {
				code: dbError.code,
				cause: dbError.context?.cause,
			});
			return { success: false, error: dbError };
