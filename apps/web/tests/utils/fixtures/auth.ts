...
export type UserRole = "admin" | "user" | "newUser";

// Authentication data for different user roles
interface UserData {
	email: string;
	password: string;
	name: string;
...
function getUserData(role: UserRole): UserData {
	const users: Record<UserRole, UserData> = {
		admin: {
...
export const test = base.extend<AuthFixtures, AuthWorkerFixtures>({
	...
	storageStatePath: [
		async (_args: any, use: any, workerInfo: any) => {
	...
	// Admin page fixture
	adminPage: async ({ browser }: { browser: any }, use: any) => {
	...
	userPage: async ({ browser }: { browser: any }, use: any) => {
		const context = await browser.newContext({
	...
	newUserPage: async ({ browser }: { browser: any }, use: any) => {
		const context = await browser.newContext();
