import { createAlert, KEY_METRIC_ALERTS, logger } from "@snapback/infrastructure";
import { z } from "zod";
...
export const setupAlerts = publicProcedure.input(setupAlertsSchema).handler(async ({ input }) => {
	try {
		const { dryRun } = input;
