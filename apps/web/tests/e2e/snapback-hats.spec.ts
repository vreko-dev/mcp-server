// tests/e2e/snapback-hats.spec.ts
import { expect, test } from "@playwright/test";

test.describe("@snapback Protection Hats Demo", () => {
	test.beforeEach(async ({ page }) => {
		await page.goto("/snapback-demo");
	});
	...
	test("should have visual snapshots", async ({ page }) => {
		...
		await expect(page).toHaveScreenshot("snapback-hats-default.png", {
		...
		await expect(page).toHaveScreenshot("snapback-hats-hover.png", {
			maxDiffPixelRatio: 0.01,
		});
		...
		await expect(page).toHaveScreenshot("snapback-hats-context-menu.png", {
			maxDiffPixelRatio: 0.01,
		});
	});
});
