...
import { existsSync, readFileSync, readdirSync, writeFileSync, mkdirSync, renameSync, rmSync, cpSync } from 'node:fs';
...
function rollback(backupPath: string, state: StateSnapshot) {
  console.log('\n🔄 Rolling back changes...');
  
  try {
    // Restore git state
    console.log('  Resetting git state...');
    exec('git reset --hard', { silent: true });
    exec('git clean -fd', { silent: true });
    
    // Restore from backup
    console.log('  Restoring from backup...');
    if (existsSync(backupPath)) {
      const files = readdirSync(backupPath, { recursive: true, withFileTypes: true });
      for (const file of files) {
        if (file.isFile()) {
          const relativePath = join(file.path.replace(backupPath, ''), file.name);
          const dest = join(process.cwd(), relativePath);
          cpSync(join(file.path, file.name), dest);
        }
      }
    }
    
    // Restore archived files if any
    if (state.archivedFiles && state.archivedFiles.length > 0) {
      console.log('  Restoring archived files...');
      for (const file of state.archivedFiles) {
        const archivePath = join('.archive', file);
        if (existsSync(archivePath)) {
          const originalPath = file.replace(/^\.archive\/[^/]+\/\d{4}-\d{2}-\d{2}\//, '');
          renameSync(archivePath, originalPath);
        }
      }
    }
...
async function runOSSSync(dryRun: boolean) {
  ...
  const turboConfig = JSON.parse(
    readFileSync(join(process.cwd(), 'turbo.json'), 'utf-8')
  );
...
async function main() {
  ...
  try {
    ...
    if (!config.skipCleanup) {
      ...
      if (!config.dryRun) saveState(currentState);
    ...
    if (error instanceof PrePublishError) {
      ...
      if (error.recoverable && backupPath && currentState) {
        ...
        try {
          ...
          console.error('\n✅ Rollback successful');
          console.error('   You can fix the issue and retry');
          process.exit(1);
        } catch (rollbackError) {
