...
describe("Phase 1: Infrastructure Split", () => {
	describe("OSS Infrastructure Package", () => {
		...
		it("should NOT contain posthog imports in source files", () => {
			...
			try {
				...
				execSync(`grep -r -i "posthog" ${join(OSS_INFRA, "src")}`, { encoding: "utf-8" });
	...
	describe("Import Updates", () => {
		...
		it.skip("private packages should import from analytics-infra for PostHog", () => {
			const packagesToCheck = ["packages/platform", "packages/analytics", "apps/api", "apps/web"];
			...
			for (const pkg of packagesToCheck) {
				...
				try {
					...
					const result = execSync(`grep -r "@snapback/infrastructure.*posthog" ${pkgPath} || true`, {
						encoding: "utf-8",
					});
