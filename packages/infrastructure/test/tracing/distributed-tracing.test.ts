	...
	describe("W3C Trace Context Compliance", () => {
		...
		it("should reject invalid traceparent header format", () => {
			...
			const invalidHeaders = [
				...
				// Note: OTel propagator is lenient with version numbers,
				// so version 99 might still extract context
			];
			...
			for (const headers of invalidHeaders) {
				...
				// Assert: Should return null for clearly invalid headers
				// (but OTel may be lenient with some formats)
				if (headers.traceparent === "invalid" || headers.traceparent === "00-tooshort-00f067aa0ba902b7-01") {
