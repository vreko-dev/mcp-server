...
class InMemoryEventStorage {
	private events: Map<string, EnhancedEvent> = new Map();

	async storeEvent(event: EnhancedEvent): Promise<void> {
...
export class SnapBackEventBusEventEmitter2 {
	...
	private requestHandlers: Map<string, (data: unknown) => Promise<unknown>> = new Map();
	...
	on(event: string | string[], listener: (...args: unknown[]) => void): this {
		this.emitter.on(event, listener);
		return this;
	}

	once(event: string | string[], listener: (...args: unknown[]) => void): this {
		this.emitter.once(event, listener);
		return this;
	}

	off(event: string | string[], listener: (...args: unknown[]) => void): this {
	...
	onRequest<TData = unknown, TResult = unknown>(event: string, handler: (data: TData) => Promise<TResult>): void {
		this.requestHandlers.set(event, handler as (data: unknown) => Promise<unknown>);
	}

	async request<TResult = unknown>(event: string, data: unknown, timeoutMs = 5000): Promise<TResult> {
