...
export function getOAuthErrorMessage(error: unknown): string {
	...
	const errorMessages: Record<string, string> = {
		...
		token_exchange_failed: "Failed to complete sign-in with provider. Please try again.",
		invalid_code: "Invalid authorization code. Please try again.",
		code_expired: "Authorization code expired. Please try again.",

		// Email verification
		email_not_verified: "Please verify your email with the provider before signing in.",
		unverified_email: "Please verify your email with the provider before signing in.",

		// Private email (GitHub)
		private_email: "Your email is private. Please allow email access in your account settings.",
		email_not_public: "Your email is private. Please allow email access in your account settings.",

		// Account linking
		account_already_linked: "This account is already linked to another user. Please use a different account.",
		social_account_already_linked:
			"This account is already linked to another user. Please use a different account.",

		// Provider errors
		provider_error: "The authentication provider is temporarily unavailable. Please try again later.",
		provider_unavailable: "The authentication provider is temporarily unavailable. Please try again later.",
		...
		provider_not_configured: "Authentication provider is not properly configured.",
		missing_credentials: "Missing provider credentials.",
	};

	// Try to match error code
	if (err.code && errorMessages[err.code]) {
		return errorMessages[err.code] ?? "Authentication failed. Please try again.";
	}
	...
	if (err.message) {
		...
		if (lowerMessage.includes("cancel") || lowerMessage.includes("denied")) {
			return errorMessages.oauth_cancelled ?? "Sign-in was cancelled. Please try again.";
		}
		if (lowerMessage.includes("state")) {
			return errorMessages.oauth_state_mismatch ?? "Security validation failed. Please try again.";
		}
		if (lowerMessage.includes("token")) {
			return errorMessages.token_exchange_failed ?? "Failed to complete sign-in with provider. Please try again.";
		}
		if (lowerMessage.includes("email") && lowerMessage.includes("verify")) {
			return errorMessages.email_not_verified ?? "Please verify your email with the provider before signing in.";
		}
		...
		if (lowerMessage.includes("network") || lowerMessage.includes("connection")) {
			return errorMessages.network_error ?? "Network connection failed. Please check your connection.";
		}
		if (lowerMessage.includes("timeout")) {
			return errorMessages.timeout ?? "Authentication timed out. Please try again.";
		}
	}
	...
	if (err.status) {
		if (err.status >= 500) {
			return errorMessages.provider_error ?? "Authentication service error. Please try again later.";
		}
