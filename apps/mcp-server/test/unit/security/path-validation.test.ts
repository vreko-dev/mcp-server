...
describe("Path Validation - Security Critical", () => {
	...
	describe("Path Traversal Attack Prevention", () => {
		...
		it("should reject ../ sequences", async () => {
			...
			expect(() => validateFilePath(maliciousPath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(maliciousPath, WORKSPACE_ROOT)).toThrow("Path traversal not allowed");
		});
		...
		it("should reject URL-encoded traversal (%2e%2e%2f)", async () => {
			...
			expect(() => validateFilePath(encodedPath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(encodedPath, WORKSPACE_ROOT)).toThrow("Encoded path traversal not allowed");
		});
	...
	describe("Absolute Path Workspace Boundary Checking", () => {
		...
		it("should reject absolute paths outside workspace", async () => {
			...
			const outsidePaths = ["/etc/passwd", "/home/user/secrets.txt", "/usr/local/bin/malicious.sh"];

			// WHEN/THEN: All should be rejected
			for (const outsidePath of outsidePaths) {
				expect(() => validateFilePath(outsidePath, WORKSPACE_ROOT)).toThrow(SecurityError);
				expect(() => validateFilePath(outsidePath, WORKSPACE_ROOT)).toThrow("Path outside workspace");
			}
		});
		...
		it("should allow paths within workspace", async () => {
			...
			for (const validPath of validPaths) {
				expect(() => validateFilePath(validPath, WORKSPACE_ROOT)).not.toThrow();
			}
		});
	});
	...
	describe("Symlink Attack Prevention", () => {
		...
		it("should reject symlink to outside workspace", async () => {
			...
			// THEN: Should reject
			expect(() => validateFilePath(symlinkPath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(symlinkPath, WORKSPACE_ROOT)).toThrow("Path outside workspace");
		});
		...
		it("should allow symlink within workspace", async () => {
			...
			// WHEN: Validating symlink
			// THEN: Should allow
			expect(() => validateFilePath(symlinkPath, WORKSPACE_ROOT)).not.toThrow();
		});
		...
		it("should reject circular symlinks with error", async () => {
			...
			// THEN: Should reject with SecurityError
			expect(() => validateFilePath(circularPath, WORKSPACE_ROOT)).toThrow(SecurityError);
		});
	});
	...
	describe("Security Violation Telemetry Logging", () => {
		...
		it("should log encoded traversal violations", async () => {
			...
			expect(() => validateFilePath(encodedPath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(encodedPath, WORKSPACE_ROOT)).toThrow("Encoded path traversal");
		});
		...
		it("should log workspace boundary violations", async () => {
			...
			expect(() => validateFilePath(outsidePath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(outsidePath, WORKSPACE_ROOT)).toThrow("outside workspace");
		});
		...
		it("should log null byte violations", async () => {
			...
			expect(() => validateFilePath(nullBytePath, WORKSPACE_ROOT)).toThrow(SecurityError);
			expect(() => validateFilePath(nullBytePath, WORKSPACE_ROOT)).toThrow("Null bytes");
		});
	});
	...
	describe("Edge Cases and Regression Prevention", () => {
		...
		it("should reject empty paths", async () => {
			...
			for (const emptyPath of emptyPaths) {
				expect(() => validateFilePath(emptyPath, WORKSPACE_ROOT)).toThrow(SecurityError);
				expect(() => validateFilePath(emptyPath, WORKSPACE_ROOT)).toThrow("cannot be empty");
			}
		});
		...
		it("should handle Windows UNC paths (security risk)", async () => {
			...
			try {
				...
				expect(() => validateFilePath(uncPath, WORKSPACE_ROOT)).toThrow(SecurityError);
				expect(() => validateFilePath(uncPath, WORKSPACE_ROOT)).toThrow("UNC paths not allowed");
			} finally {
				if (originalPlatform) {
					Object.defineProperty(process, "platform", originalPlatform);
				}
		...
		it("should handle Windows drive letter attacks", async () => {
			...
			try {
				...
				for (const drivePath of drivePaths) {
					expect(() => validateFilePath(drivePath, WORKSPACE_ROOT)).toThrow(SecurityError);
					expect(() => validateFilePath(drivePath, WORKSPACE_ROOT)).toThrow("drive letters not allowed");
				}
			} finally {
				if (originalPlatform) {
					Object.defineProperty(process, "platform", originalPlatform);
