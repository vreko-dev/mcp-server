...
describe.skipIf(!isDatabaseAvailable)("AUTH-OPT: Better Auth Optimizations Integration", () => {
	let auth: any;
	...
	describe("OPT1: Cookie Cache Configuration", () => {
		...
		it("CRITICAL: should have automatic cache refresh configured", () => {
			const refreshConfig = (auth as any).options?.session?.cookieCache?.refreshCache;
	...
	describe("OPT3: Organization RBAC", () => {
		it("CRITICAL: should have access control configured", async () => {
			const { ac } = await import("../../src/lib/organization-permissions.js");
		...
		it("CRITICAL: should enforce role permissions", async () => {
			...
			expect((admin as any).permissions?.organization || []).not.toContain("delete");

			// Member should only have read permissions
			expect((member as any).permissions?.snapshot).toContain("read");
			expect((member as any).permissions?.snapshot || []).not.toContain("delete");
		});
	...
	describe("OPT4: ID Generation Strategy", () => {
		...
		it("CRITICAL: should generate unique IDs", async () => {
			const generateId = (auth as any).options?.advanced?.database?.generateId;
	...
	describe("OPT6: Enhanced Cookie Security", () => {
		it("should have secure cookie configuration", () => {
			const cookieConfig = (auth as any).options?.advanced?.defaultCookieAttributes;
		...
		it("should have cross-subdomain cookies for production", () => {
			const crossSubdomain = (auth as any).options?.advanced?.crossSubDomainCookies;
	...
	describe("PERFORMANCE: Overall System Performance", () => {
		it("should reduce database load with cookie cache", () => {
			...
			const cookieCacheEnabled = (auth as any).options?.session?.cookieCache?.enabled;
	...
	describe("SECURITY: Production Security Posture", () => {
		...
		it("should track IP addresses for audit", () => {
			const ipTracking = (auth as any).options?.advanced?.ipAddress?.disableIpTracking;
			expect(ipTracking).toBe(false);
		});

		it("should enforce CSRF protection", () => {
			const csrfConfig = (auth as any).options?.advanced?.crossSiteRequestForgery;
		...
		it("should use secure cookies in production", () => {
			const useSecureCookies = (auth as any).options?.advanced?.useSecureCookies;
	...
	describe("EDGE-CASES: Resilience and Error Handling", () => {
		...
		it("should handle missing IP headers", () => {
			...
			const headers = (auth as any).options?.advanced?.ipAddress?.ipAddressHeaders;
...
describe("SUMMARY: Optimization Verification Checklist", () => {
	...
	it("✅ Redis secondary storage configured for distributed systems", async () => {
		const { auth } = await import("../../src/auth.js");
		const hasSecondaryStorage = Boolean((auth as any).options?.secondaryStorage);
		const hasRedis = Boolean(process.env.REDIS_URL);
	...
	it("✅ Organization RBAC configured with roles and permissions", async () => {
		const { ac, owner, admin, member } = await import("../../src/lib/organization-permissions.js");
	...
	it("✅ Consistent ID generation using cuid2", async () => {
		const { auth } = await import("../../src/auth.js");
		expect(typeof (auth as any).options?.advanced?.database?.generateId).toBe("function");
	});

	it("✅ IP tracking configured for security audit", async () => {
		const { auth } = await import("../../src/auth.js");
		expect((auth as any).options?.advanced?.ipAddress?.disableIpTracking).toBe(false);
	});
