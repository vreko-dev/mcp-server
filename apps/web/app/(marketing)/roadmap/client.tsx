      ...
      <div className="container max-w-4xl mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-gray-400">
            Building the safety layer for the AI age.
          </p>
        </m.div>
                  ...
                  <ul className={`space-y-2 ${index % 2 === 0 ? "md:items-end" : "md:items-start"} flex flex-col`}>
                    {milestone.items.map((item) => (
                      <li key={item} className="text-gray-400 text-sm flex items-center gap-2">
                         {index % 2 !== 0 && <ArrowRight className="w-3 h-3 text-emerald-500/50" />}
                         {item}
                         {index % 2 === 0 && <ArrowRight className="w-3 h-3 text-emerald-500/50 rotate-180" />}
                      </li>
                    ))}
                  </ul>
                </div>
