...
app.post("/keys", requireStepUp, requirePasskey, zValidator("json", createKeySchema), async (c) => {
	try {
...
app.post("/keys/revoke", requireStepUp, requirePasskey, zValidator("json", revokeKeySchema), async (c) => {
	try {
		// Get authenticated user
		const authResult = await auth.api.getSession({
			headers: c.req.raw.headers,
