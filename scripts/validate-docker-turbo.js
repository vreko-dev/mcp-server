...
const requiredDockerfiles = ["apps/api/Dockerfile.prod", "apps/mcp-server/Dockerfile.prod", "apps/web/Dockerfile.prod"];
...
for (const { path: pkgPath, name } of packagesToCheck) {
	...
	if (fs.existsSync(filePath)) {
		...
		const hasDockerBuildDev = pkg.scripts?.["docker-build:dev"];
